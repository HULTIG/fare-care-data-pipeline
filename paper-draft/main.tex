...
\end{itemize}
These datasets provide a representative evaluation across multiple high-risk application contexts. Their characteristics are summarized in Table \ref{tab:datasets}.
\begin{table}[ht]
\caption{Dataset Characteristics}
\label{tab:datasets}
\begin{tabular}{l l l l}
\toprule
\textbf{Dataset} & \textbf{N} & \textbf{Target} & \textbf{Sensitive Attributes} \\
\midrule
NIJ Recidivism & 25,835 & Recidivism (29.9\%) & Race (57\% B, 43\% W), Gender (88\% M) \\
COMPAS & 6,172 & Recidivism (45\%) & Race (51\% B, 34\% W), Sex \\
Adult Census & 32,561 & Income $>$50k (24\%) & Sex (67\% M), Race (85\% W) \\
German Credit & 1,000 & Risk (30\% High) & Age, Sex \\
\bottomrule
\end{tabular}
\end{table}
This selection enables measurement of fairness under varying conditions of class imbalance and sensitive attribute distribution. Figure \ref{fig:eda} illustrates the demographic distributions and imbalances across the four benchmark datasets.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{fig_demographic.png}
...
Experiments were executed using a \textbf{hybrid PySpark pipeline}. Spark handles high-throughput ingestion and Bronze-layer transformations, while modular Python components perform causal discovery, privacy verification, and fair-metric calculation.
...
We applied the full FAIR-CARE pipeline to three widely used fairness datasets: COMPAS, Adult Census, and German Credit. Across all benchmarks, the architecture substantially reduced fairness disparities relative to a naive ETL baseline.

Table~\ref{tab:ablation} presents an ablation study on the Adult Census dataset. The full FAIR-CARE configuration (Config B) increases the composite score from a baseline of 0.72 to 0.78. This improvement is driven by the formal privacy guarantees of Differential Privacy ($\epsilon=1.0$) and high utility retention (Utility $\approx 1.0$), despite a trade-off in raw performance.
...
A paired t-test was conducted across the four datasets to evaluate the improvement of the FAIR-CARE score relative to the baseline. For Config A ($k$-anonymity), the test yielded $p = 0.23$ ($t = -1.49$). For Config B (Differential Privacy), $p = 0.11$ ($t = -2.25$). These results indicate that while the architecture provides structural guarantees, the difference in the composite score is not statistically significant at the $p < 0.05$ level, likely due to the limited sample size ($N=4$ datasets). However, the reduction in privacy risk (an explicit boolean constraint in our score) remains a deterministic improvement not fully captured by the mean score comparison.
...
\begin{itemize}
    \item \textbf{Baseline (Legacy):} Direct modeling on raw data yielded a high AUC of 0.92, indicating strong predictive signal in the raw features, but with undefined privacy retention.
    \item \textbf{Silver Layer (Privacy):} Applying the Privacy Preservation module with $\epsilon = 1.0$ maintained a robust AUC of 0.88, demonstrating that utility can be largely preserved (approx 96\% retention) even under strict privacy constraints.
    \item \textbf{Gold Layer (Fairness):} After bias mitigation via AIF360 Reweighing, we achieved an improved composite FAIR-CARE Score ($0.74 \rightarrow 0.77$), validating the architecture's ability to balance fairness corrections with sustained utility (Final AUC $\approx$ 0.88).
\end{itemize}

Although the FAIR-CARE pipeline does not maximize predictive accuracy, it produces an \textit{ethically robust} model with demonstrable protections against discriminatory or privacy-violating behavior, an essential property for correctional applications.
