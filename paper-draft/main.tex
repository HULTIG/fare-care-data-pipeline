...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.9\textwidth]{images/fair-care-arch.png}
...
In high-risk environments where AI systems’ decisions have direct social, legal, or safety implications, they are a critical part of the architecture.
Because they mediate how information is collected, transformed, and represented in downstream models. Ensuring the trustworthiness of these systems therefore requires going beyond traditional functional ETL data pipelines and explicitly managing non-functional requirements (NFRs) such as fairness, privacy, lineage integrity, and auditability.\cite{li2021trustworthy, chouldechova2017fair}. Correctional datasets exemplify the challenges of high-risk governance. Recidivism forecasting models are susceptible to ``data bias,'' where patterns reflect historical policing practices rather than behavioral risk \cite{nij_context}. Privacy risks are equally substantial: combinations of quasi-identifiers such as release dates, geographic zones, and offense categories can re-identify individuals even after direct identifiers are removed \cite{sweeney2002k}. These issues highlight that ethical and regulatory requirements must be operationalized at the architectural level, not delegated to model developers or compliance teams in isolation.

\subsection{Problem Statement}
Although traditional Extract-Transform-Load (ETL) and Lakehouse architectures are optimized for throughput, schema evolution, and analytical readiness, they are poorly designed to enforce ethical constraints throughout the data lifecycle. Thus, the following limitations are clearly visible:
...
Therefore, designing a unified architecture that can integrate fairness, privacy, lineage management, and causal reasoning as first-class architectural citizens is a fundamental need.
The solution to overcoming these gaps is to redefine the role of the software architect as the custodian of data ethics and responsive system behavior.
...
\begin{table}[ht]
    ...
    % \columnwidth ensures it exactly fits the column width
    % The 'X' column type automatically wraps long text
    \begin{tabularx}{\columnwidth}{@{}l c c l X@{}}
        \toprule
        \textbf{Dataset} & \textbf{Size} & \textbf{Pos.} & \textbf{Sens.} & \textbf{Primary Bias} \\
        \midrule
        NIJ Recid. & 25,690 & 30\% & Race, Gen. & Gender skew (88\% Male), Racial imbalance (57\% Black) \\
        \midrule
        COMPAS & 7,214 & 45\% & Race, Sex & Racial disparity (52\% African-Amer., 34\% Cauc.) \\
        \midrule
        Adult Census & 32,561 & 24\% & Race, Sex & Race-income corr. (85\% White, skewed $\le$50K) \\
        \midrule
        Germ. Credit & 1,000 & 30\% & Age, Sex & Small sample, age skew (mean=35), 69\% Male \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/fig_demographic.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/fig3_benchmark_datasets.png}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\columnwidth}{@{}X c c c c@{}}
        \toprule
        \textbf{Configuration} & \textbf{Score}$_{FC}$ & \textbf{SS} & \textbf{Privacy} & \textbf{Utility} \\
        \midrule
        ...
        \bottomrule
        \multicolumn{5}{@{}l}{\scriptsize * SS denotes Silver Score component.}
    \end{tabularx}
\end{table}
...
In the NIJ Recidivism Forecasting Challenge dataset, we model the relationships between sensitive attributes, supervision practices, and outcomes using the causal diagram in Figure~\ref{fig:nij_causal_dag}. The highlighted path from race to recidivism via supervision level reflects a potential channel through which disparate supervision intensity, rather than underlying risk, can induce structural bias in observed outcomes.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\columnwidth]{images/fig_nij_causal_graph.png}
    \caption{Causal DAG for the NIJ Recidivism dataset highlighting a potential unfair path from race to recidivism via supervision level. The red edge denotes a suspected bias pathway that the proposed data pipeline aims to monitor and mitigate.}
    \label{fig:nij_causal_dag}
\end{figure}

To assess the architecture in a high-risk, real-world context, we evaluated the NIJ Recidivism Forecasting Challenge dataset~\cite{nij_challenge}, using Logistic Regression and ROC AUC as the primary utility metric and Statistical Parity Difference / Equalized Odds Difference as fairness metrics. Contest organizers emphasize the importance of limiting disparities in error rates across racial groups, reflecting the harm of disproportionate false positives in supervision and sanctioning decisions.

\begin{itemize}
    \item \textbf{Baseline (Legacy):} Direct modeling on raw data achieved a high AUC of 0.92 but exhibited substantial privacy leakage and large group disparities (e.g., SPD and EOD both around 0.18), indicating strong predictive signal at the cost of fairness.
    \item \textbf{Silver Layer (Privacy):} Applying the Privacy Preservation module with $\epsilon = 1.0$ (a strict, regulation-aligned privacy budget) maintained a robust AUC of 0.91, showing that most predictive utility can be preserved under differential privacy.
    \item \textbf{Gold Layer (Fairness):} After bias mitigation via AIF360 Reweighing, the model achieved a marked reduction in fairness disparity (SPD and EOD reduced to approximately 0.07) with only a minor decrease in AUC (final AUC $\approx 0.91$), demonstrating that the architecture can attenuate historical bias while largely retaining predictive performance.
\end{itemize}

Although the FAIR-CARE pipeline does not maximize predictive accuracy, it yields a more \emph{ethically robust} model that explicitly trades a small amount of utility for stronger protections against discriminatory and privacy-violating behavior, consistent with emerging principles for trustworthy, high-risk AI systems. This trade-off is summarized in Figure~\ref{fig:nij_case_study}, which visualizes the joint evolution of AUC and fairness metrics across the Baseline, Silver, and Gold configurations.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{images/fig_nij_case_study.png}
    \caption{Case study results on the NIJ Recidivism dataset. The chart illustrates the trade-off between predictive utility (AUC) and fairness disparity (e.g., Equalized Odds Difference). While the Baseline model achieves high AUC (0.92), it suffers from high disparity (EOD $\approx 0.18$). The Gold Layer sacrifices a small amount of predictive power (AUC $\approx 0.91$) to obtain a substantially fairer model (EOD $\approx 0.07$).}
...
One of the important developments of the FAIR-CARE architecture is its integration with federated learning (FL) to support distributed model development and privacy across organizational boundaries. Correction data is typically distributed across jurisdictions, each subject to distinct legal, operational, and infrastructure constraints. Centralizing such data is often impractical due to trust concerns between organizations and regulatory constraints (e.g., GDPR data minimization). By leveraging federated learning frameworks such as NVFlare, Flower, or Ray Fed, collaborative learning can be enabled without moving raw data, thereby enhancing the architectural privacy guarantees. From an architectural perspective, federated learning offers new dimensions of robust governance. Privacy budgets must be managed across federated rounds, fairness must be monitored for participating nodes, and causal structures may differ across data silos. Future work could explore extending the FAIR-CARE score to a federated governance score that reflects heterogeneity, deviations, and ethical compliance across sites. Automated governance mechanisms such as node-level anomaly detection, fair deviation monitoring, and DP-aware aggregation are essential to maintain ethical guarantees in asynchronous and competitive training environments. Therefore, integrating FL with Lakehouse FAIR-CARE is not only a technical advancement but also a natural step towards a continuous architecture in high-risk AI ecosystems, precisely where data sources evolve independently, and governance needs to adapt dynamically.
